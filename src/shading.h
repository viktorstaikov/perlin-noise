...
class PerlinNoise: public Texture {
    ...
    {
        int x = (int)q.components[0] & 255;
        int y = (int)q.components[1] & 255;

        return gradient[x + permutation[y]];
    }

    double s_curve(const double t)
    {
        return 3*t*t - 2*t*t*t;
    }

    double interpolateLinear(const double a, const double b, const double t)
    {
        return a*(1-t) + t*b;
    }
