...
Scene::~Scene()
{
	...
	for (int i = 0; i < (int) superNodes.size(); i++) {
		if (superNodes[i]) delete superNodes[i];
	}
	superNodes.clear();
...
{
	frameWidth = DEFAULT_RESOLUTION_WIDTH;
	frameHeight = DEFAULT_RESOLUTION_HEIGHT;
...
SceneElement* DefaultSceneParser::newSceneElement(const char* className)
{
	...
	if (!strcmp(className, "Reflection")) return new Refl;
	if (!strcmp(className, "Refraction")) return new Refr;
	...
	if (!strcmp(className, "PerlinNoise")) return new PerlinNoise;
	return NULL;
}
