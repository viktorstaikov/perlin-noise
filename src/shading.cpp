...
PerlinNoise::PerlinNoise()
{
    ...
    for(int i = 0; i< 256; i++) permutation[i + 256] = permutation[i] = i;

    //shuffle using Knuth-Fisher-Yates shuffle algorithm
    //http://www.codinghorror.com/blog/2007/12/the-danger-of-naivete.html
    for(int i = 511; i>0; i--){
        int n = rnd.randint(0, i);
        //this is bitwise swap
        permutation[i]^=permutation[n]^=permutation[i]^=permutation[n];
    }
    ...
    {
        double _x,_y;
        rnd.unitDiscSample(_x,_y);

        int x = (int)(fabs(_x)*255);
        int y = (int)(fabs(_y)*255);

        gradient[i+256] = gradient[i] = Vector(permutation[x], permutation[y],0);
    }
}
